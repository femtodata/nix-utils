...
let
  ...
  composeFile = name: service: if service.composeContents != {}
...
in {
        ...
        type = types.attrsOf (types.submodule ({ name, ...}: {
          options = {
            ...
            additionalFiles = lib.mkOption {
              ...
              default = [];
              description = lib.mdDoc ''
                Additional file paths to copy into service directory. If envSubstFile
                is defined, substitutions of `$VAR` will be replaced with definitions
                found therein.
              '';
            };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services = mapAttrs'
      (name: value: nameValuePair "generate-docker-compose-${name}" {
        ...
        preStart = ''
          ...
          '' + lib.optionalString (value.envSubstFile != null) ''
            set -o allexport
            source ${value.envSubstFile}
            set +o allexport
          '' + ''
